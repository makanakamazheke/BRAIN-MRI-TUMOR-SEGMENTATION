...
% Displaying the representative slice

figure;% creating figure object
imagesc(y_axis,x_axis,axial_slice_15) % displaying the slice via imagesc using adjusted axes
...
title ('Representative Slice 15 for Axial (XY Plane)') % Title of the representative slice image
...
% Tumors have high intensities due to their high water content, as they contribute more to the MRI than healthy brain tissue
% The histogram shows a small peak at the high intensity range, starting at around 0.97 (since the intensities are normalised) 
% I also noted that this particular peak at high intensities is absent in the task 2 MRI image without a tumor embedded
...
% I'm applying the lower limit only, as the upper limit is 1(maximum possible intensity)
...
lower_limit_line_2 = xline(lower_limit,'-','Lower Limit', 'Color','r','LabelHorizontalAlignment','left',...
    'LabelVerticalAlignment','middle','LineStyle','--', 'FontSize',9); % lower limit line
upper_limit_line_2 = xline(upper_limit,'-','Upper Limit', 'Color','r','LabelHorizontalAlignment','left', ...
    'LabelVerticalAlignment','middle', 'LineStyle','--', 'FontSize',9); % upper limit line

%% STEP 2 Part 1: 3D volume segmentation 

% Same process as in Step 1, but in 3D
% The same logic for the 2D tumor mask applies (tumor corresponds to the high intensity peak)
...
% This function is found in the dice_function.m file
...
figure;  % creating figure object
hold on % hold + patch + set for tumor, then repeat the whole brain data
...
title('3D Volume Rendering Of The Tumor') % Title for 3D Volume Rendering of Tumor image
